...
```bash
docker build --target development --no-cache --tag ras-queue-backend:development .
```

Run:

```bash
docker run -e PORT=3001 -p 3001:3001 ras-queue-backend:development
```
...
```bash
docker build --no-cache --tag ras-queue-backend .
```

Run:

```bash
docker run -e PORT=3001 -p 3001:3001 ras-queue-backend
```
